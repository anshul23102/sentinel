    ...
    <div>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 14 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          <span style={{ fontSize: 11, fontWeight: 600, color: 'var(--text-35)', textTransform: 'uppercase', letterSpacing: '0.8px' }}>
            Anomaly Feed
          </span>
          {anomalies.length > 0 && (
            <span style={{
      ...
      {anomalies.length === 0 ? (
        <div style={{
          ...
          <div style={{ fontSize: 12, color: 'var(--text-30)', marginTop: 4 }}>
...
function AnomalyRow({ anomaly, analysis }) {
  ...
  return (
    <div style={{
      background: open ? `var(--bg-04)` : 'var(--bg-025)',
      border: `1px solid ${open ? t.color + '30' : 'var(--text-07)'}`,
      ...
      <div onClick={() => setOpen(!open)} style={{
        ...
        {/* Endpoint */}
        <span style={{ fontSize: 12, fontFamily: 'SF Mono, monospace', color: 'var(--text-70)', flexShrink: 0 }}>
          {anomaly.endpoint === 'multiple' ? 'multiple' : anomaly.endpoint}
        </span>

        {/* Description */}
        <span style={{
          fontSize: 12, color: 'var(--text-35)', flex: 1,
          overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap',
        }}>{anomaly.description}</span>

        {/* Time + chevron */}
        <span style={{ fontSize: 11, color: 'var(--text-25)', flexShrink: 0 }}>
          {anomaly.detected_at?.slice(11, 19)}
        </span>
        <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="var(--text-30)" strokeWidth="2"
        ...
        <div style={{ borderTop: '1px solid var(--text-06)', padding: 16 }}>
          ...
          {anomaly.root_cause_chain?.length > 0 && (
            <div style={{ marginBottom: 14 }}>
              <div style={{ fontSize: 10, fontWeight: 600, color: 'var(--text-30)', textTransform: 'uppercase', letterSpacing: '0.8px', marginBottom: 10 }}>
                Root Cause Chain
              </div>
                ...
                {anomaly.root_cause_chain.map((step, i) => (
                  <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                    <div style={{
                      ...
                      <div style={{ fontSize: 12, fontWeight: 600, color: 'var(--text-85)' }}>{step.component}</div>
                      <div style={{ fontSize: 11, color: 'var(--text-40)', marginTop: 2 }}>{step.signal}</div>
          ...
          {analysis ? (
            <div style={{
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 10 }}>
                ...
                <span style={{ fontSize: 12, fontWeight: 600, color: 'var(--text-85)' }}>AI Diagnosis</span>
                <span style={{ fontSize: 10, color: 'var(--text-25)', marginLeft: 'auto' }}>
                  {analysis.model} · {analysis.analyzed_at?.slice(11, 19)}
                </span>
              </div>
              <pre style={{
                fontSize: 12, color: 'var(--text-62)', lineHeight: 1.7,
